...
$\Vc{B}$ matrix. We are working with {\em man made} block sparsity:
for example, the block is a property of the hardware ans not of the
application.
...
have to find a way to chose the binary matrix $\Gamma$, the mask. For
...
relative block is non zero, nothing otherwise, and do the same for
...
also the case where the compiler failes to generate code and thus
estiamtes. A $4\times 4$ cohort can be replicated up to 40 times
(think as a batch of 40) and we can do better when the weight are
shared into a single memtiles for each 40 copies and separated from
activations (there is such design).

Here We present the collection of execution times as images: 
\begin{itemize}
