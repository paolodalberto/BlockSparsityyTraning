...
convolution and one column to max-pool. Other approach in the
literature will do this more advanced methodology.
...
a final address and weights are distributed similarly with a start and
...
A important feature of the current system is the concept of {\bf
...
\DDoublefigure{0.95}{Iv36x6-6shareddense.png}{Iv36x6-6sharedsparse.png}{Inception V3 for 6x6 AIE
  with dense (left)  weights and sparse (right) }{fig:incv3-estimate-dense}
...
This work in particular stemmed from a collaboration between Xilinx
and Numenta and the idea set forward by the authors in
\cite{ahmad2019dense}. Sparsity at the beginning was proposed for
weights with blocks $4\times 4$and for activations (a ReLU zeros a lot
and there have been prototypes programming logic was added to AIE1
fabric to support top-k run time activation). We must thanks Vamsi
Nalluri for the AIE1 prototype and the original contribution to the
AIE2. With the new AIE2 and logic, the run time support and top-k was
dropped. The compiler could and can do average estimate of tensor
contents but we do not have run time support for dynamic and
unstructured sparsity. New support are coming like the 2 over 4 (4
over 8) sparsity but it is not clear how it will affect the activation
encoding and total accuracy. In principle, we could use 2 by 4 zero
encoding for the non zero blocks to further reduce space and
computations.
...
\section{appendix}
We present the collection of execution times as images:
\begin{itemize}
...
\DDoublefigure{1.3}{IncV3-2x2-2share-dense.png}{IncV3-2x2-2share-sparse.png}{Inception for 2x2 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-1}
\DDoublefigure{1.3}{IncV3-3x3-3share-dense.png}{IncV3-3x3-3share-sparse.png}{Inception for 3x3 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-2}
\DDoublefigure{1.3}{IncV3-4x4-4share-dense.png}{IncV3-4x4-4share-sparse.png}{Inception for 4x4 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-3}
\SSinglefigure{2}{IncV3-5x5-5share-dense.png}{Inception for 5x5 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-4}
\DDoublefigure{1.3}{IncV3-6x6-6share-dense.png}{IncV3-6x6-6share-sparse.png}{Inception for 6x6 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-5}
\DDoublefigure{1.3}{IncV3-8x8-8share-dense.png}{IncV3-8x8-8share-sparse.png}{Inception for 8x8 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-6}

\DDoublefigure{1.3}{Res2x2-2share-dense.png}{Res2x2-2share-sparse.png}{Resnet for 2x2 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-7}
\DDoublefigure{1.3}{Res3x3-3share-dense.png}{Res3x3-3share-sparse.png}{Resnet for 3x3 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-8}
\DDoublefigure{1.3}{Res4x4-4share-dense.png}{Res4x4-4share-sparse.png}{Resnet for 4x46 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-9}
\SSinglefigure{2}{Res5x5-5share-sparse.png}{Resnet for 5x5 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-10}
\DDoublefigure{1.3}{Res6x6-6share-dense.png}{Res6x6-6share-sparse.png}{Resnet for 6x6 AIE  with dense (left)  weights and sparse (right) }{fig:incv3-11}
