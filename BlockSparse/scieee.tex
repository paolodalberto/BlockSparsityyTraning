...
\author{
  ...
  \and
  \IEEEauthorblockN{Yaswanth Raparti,}
  \and
  \IEEEauthorblockN{ and Nitesh Pipralia}
}
...
We take VGG and we instruct the DDR to be 16 times more expensive. In
Figure \ref{vgg16overall}, we show the performance for VGG16 until
the last maxpool. In Figure \ref{vggddronly}, we highlight only the
part that require DDR spills for each layer: 0.025s.
\begin{figure}[htb]
\centering 
...
even. With two tiles we achieve better performance at 0.022s in Figure
\ref{vgg3tiles} and \ref{vgg2tiles}. We finally show that sparsity
improves even further 0.014s in Figure \ref{vgg2tilessparse}. Sparsity
by itself can achieve only 0.021s.
\begin{figure}[htb]
\centering 
