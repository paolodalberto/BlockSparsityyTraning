...
\title{Weight Block Sparsity: Training, Compilers, and FPGA-AIE
  Accelerators }
...
Norm). Then we train the model till convergence or accuracy
...
is a submatrix.  We provide software to reproduce this []. %\cite{PaoloG2020}.
...
bits and the size $8 \times 8$ will exploit the maximum throughput
(256 multiply-add per cycles per AIE core).


\section{Block Sparsity: Training and Quantization}
\label{sec:training}

The block sparsity is not naturally recurring.  We must train the
...
\ref{fig:singleconvestimate2}.  First, we load the weight and
...
helps only in space saving and may be spilling. In principle, we could
relax sparsity requirements for those convolutions that are
