...
\begin{equation}
  ...
  Conv(\Vc{X}_w,\Vc{W})
\end{equation}
The split is pre-computed as function of the footprint, before and
...
a tree. If the convolution has strides and large kernel, each
sub-convolution has overlap data; however, the sub-convolution has
defined addresses and data movements. For example, computing the
output by rows and the weights are reused.

Note scheduling and memory allocation among layers is a hard problem
and often barely addressed by other systems. We addressed as our first
requirements.

\subsection{Code Generation }
The compiler creates recursively a list of operations. These
operations are smaller and smaller and they can be executed memtile to
memtile. There is a further decomposition using only AIE cores and it
is completely determined by the subvolume computation. Here, we show
how we produce the execution time estimates as in Figure
...
core (as a SIMD instruction) and driving all cores at once (ASM-like
code) for multiple iterations ($\dot{\sum}_{h=1}^i Conv(\Vc{X}_w)$):
...
\singlefigure{0.8}{singledenseconv.png}{Resnet single convolution with
  padding for 4x4: legend AIE: LOAD Activation DDR to Mem, LOADW
...
Figure \ref{fig:singleconvestimate}. %\ref{fig:estimate-dense}.
...
allocation; however,  we do not allow the streaming. % as \cite{abs-2110-04327}.
